...
//     -Modified getPushpinIconIndex to show colors based on satellite status
//  2011/02/16  David Aldridge
//     -Added reason column
//     -Added rtt column
//     -Added snrCalDown
//     -Added snrCalUp
    ...
    implements EventDataProvider, GeoPointProvider
{
    ...
    // reason
    public static final String FLD_rtt			= "rtt";
    // rtt
    public static final String FLD_snrCalDown		= "snrCalDown";
    // snrCalDown
    public static final String FLD_snrCalUp		= "snrCalUp";
    // snrCalUp
    public static final String FLD_powerDbm		= "powerDbm";
    // powerDbm
    private static final DBField StandardFieldInfo[] = {
        ...
        new DBField(FLD_reason         , String.class  , DBField.TYPE_TEXT        , "Reason"                     , ""),
	new DBField(FLD_rtt	       , Double.TYPE   , DBField.TYPE_DOUBLE      , "RTT"                        , ""),
	new DBField(FLD_snrCalDown     , Double.TYPE   , DBField.TYPE_DOUBLE      , "SNR Down"                   , ""),
	new DBField(FLD_snrCalUp       , Double.TYPE   , DBField.TYPE_DOUBLE      , "SNR Up"                     , ""),
	new DBField(FLD_powerDbm       , Double.TYPE   , DBField.TYPE_DOUBLE      , "Power dbm"                  , ""),
        // Common fields
    ...
    **/
        ...
        boolean isFleet, BasicPrivateLabel bpl)
    {
        ...
        if (isFleet) {
 	    ...
 	    Device dev = this.getDevice();
            long code = dev.getLastInputState();
	    int iconNdx = 0;
            //Print.logInfo("Device '" + this.getDeviceID() + "' - statusCode " + code);
	    if (code == 39999L) {
		iconNdx = EventData.getPushpinIconIndex(devIcon,iconKeys,ICON_PUSHPIN_GRAY);
            } else
            if (code == 40000L) {
                //Print.logInfo("Green Pin");
                iconNdx = EventData.getPushpinIconIndex(devIcon, iconKeys, ICON_PUSHPIN_GREEN);
            } else
            if (code == 40001L) {
                //Print.logInfo("Yellow Pin");
                iconNdx = EventData.getPushpinIconIndex(devIcon, iconKeys, ICON_PUSHPIN_YELLOW);
            } else {
                //Print.logInfo("Red Pin");
                iconNdx = EventData.getPushpinIconIndex(devIcon, iconKeys, ICON_PUSHPIN_RED);
            }
